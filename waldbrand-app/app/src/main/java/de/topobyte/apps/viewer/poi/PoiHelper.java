...
public class PoiHelper
{
  ...
  public static boolean isPeakOrVolcano(PoiTypeInfo typeInfo, TIntSet types)
  {
    if (types == null) {
      return false;
    }
    TIntIterator peaks = typeInfo.getPeakIds().iterator();
    while (peaks.hasNext()) {
      int peakId = peaks.next();
      if (types.contains(peakId)) {
        return true;
      }
    }
    return false;
